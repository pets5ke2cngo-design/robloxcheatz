...
import { useAnimationReady } from './AnimationReadyProvider';
...
const TargetCursor: React.FC<TargetCursorProps> = ({
  ...
  const { isReady, registerComponent, markComponentReady } = useAnimationReady();
  const componentId = useRef('target-cursor-' + Math.random().toString(36).substr(2, 9));
  ...
  useEffect(() => {
    ...
    requestAnimationFrame(() => {
      markComponentReady(componentId.current);
    });
  }, [isMobile, registerComponent, markComponentReady]);
  ...
  useEffect(() => {
    if (isMobile || !cursorRef.current || !isReady) return;
  ...
  }, [targetSelector, spinDuration, moveCursor, constants, hideDefaultCursor, isMobile, hoverDuration, parallaxOn, isReady]);

  useEffect(() => {
    if (isMobile || !cursorRef.current || !spinTl.current || !isReady) return;
  ...
  return (
    <div
      ...
      style={{ willChange: 'transform', opacity: isReady ? 1 : 0, transition: 'opacity 0.3s ease' }}
    >
      <div
        ref={dotRef}
