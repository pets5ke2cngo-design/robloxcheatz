...
const ElectricBorder: React.FC<ElectricBorderProps> = ({
  ...
  const componentId = useRef('electric-border-' + rawId);
  ...
  useEffect(() => {
    ...
    updateAnim();
  }, [speed, chaos, animationStarted]);

  useLayoutEffect(() => {
    if (!rootRef.current || !animationStarted) return;
  ...
  const strokeStyle: CSSProperties = {
    ...
    borderStyle: 'solid',
    borderColor: color,
    filter: animationStarted ? `url(#${filterId})` : 'none',
  ...
  return (
    <div ref={rootRef} className={'relative isolate ' + (className ?? '')} style={{ ...style, willChange: 'transform', transform: 'translateZ(0)' }}>
      <svg
        ref={svgRef}
